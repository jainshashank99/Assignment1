...
public class vis_cluster_degree extends JPanel {
	...
	public vis_cluster_degree(Graph g, String label) {
		...
		ColorAction nStroke = new ColorAction("graph2.nodes", VisualItem.STROKECOLOR);
	...
	public static void setUpData() throws IOException {
		...
		graph2.addColumn("degree", Integer.class);
		while (!(stack.isEmpty())) {
		...
		for(int i=0;i<graph2.getNodeCount();i++) {
			Node n = graph2.getNode(i);
			n.set("degree",n.getDegree()/2);
		}
	...
	public static JFrame demo(Graph g, String label) {
		final vis_cluster_degree view = new vis_cluster_degree(g, label);
